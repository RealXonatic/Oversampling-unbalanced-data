...
def plot_pca_and_metrics_with_random_oversampler(df, n_iterations=20):
    ...
    # Plotting decision regions before Random Oversampler
    plt.subplot(1, 2, 1)
    plot_decision_regions(X_reduced, y, clf=best_model_lr, legend=2)
    plt.title('Régions de décision avant Random Oversampling')

    # Plotting decision regions after Random Oversampler
    plt.subplot(1, 2, 2)
    plot_decision_regions(X_res_pca, y_res, clf=classifier, legend=2)
    plt.title('Régions de décision après Random Oversampling')
