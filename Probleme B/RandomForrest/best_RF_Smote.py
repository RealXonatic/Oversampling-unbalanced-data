...
def plot_pca_and_metrics_with_smote(df, n_iterations=20):
    ...
    for i in range(n_iterations):
        ...
        # Parameter grid for Random Forest without SMOTE
        param_grid_rf = {
        ...
        # Cross-validation for Random Forest without SMOTE
        ...
        # Training and finding the best Random Forest model without SMOTE
        grid_search_rf.fit(X_train_pca, y_train)

        # Training the best Random Forest model without SMOTE on all training data
        best_model_rf = grid_search_rf.best_estimator_
        best_model_rf.fit(X_train_pca, y_train)

        # Predicting and evaluating on test data for Random Forest without SMOTE
        y_pred_rf = best_model_rf.predict(X_test_pca)

        # Calculating confusion matrix and accuracies after Random Forest without SMOTE
        cm_after_rf = confusion_matrix(y_test, y_pred_rf, labels=[1, 2])
        accuracy_after_rf = accuracy_score(y_test, y_pred_rf)

        # Calculating accuracy for each class after Random Forest without SMOTE
        ...
        all_results_before_smote.append(results_after_rf)

        # Calculate class counts for y_train
        class_counts = np.bincount(y_train)

        # Defining the pipeline for SMOTE + Random Forest
        pipeline = Pipeline([
            ('smote', SMOTE(random_state=42)),
            ('classifier', RandomForestClassifier(random_state=42))
        ])

        # Initial parameter grid for SMOTE + Random Forest
        param_grid = {
        ...
        # Cross-validation for SMOTE + Random Forest
        grid_search = GridSearchCV(pipeline, param_grid, cv=5, scoring=scorer, n_jobs=-1, return_train_score=True)
        grid_search.fit(X_train, y_train)

        # Training the best model SMOTE + Random Forest on all training data
    ...
    plt.tight_layout()
    plt.savefig('visualisation_pca_smote_random_forest.png')
    plt.show()
...
print("Results saved to 'combined_results_smote.csv' and PCA visualizations saved as 'visualisation_pca_smote_random_forest.png'")